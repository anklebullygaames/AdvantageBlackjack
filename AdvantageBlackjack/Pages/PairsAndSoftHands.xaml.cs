...
public partial class PairsAndSoftHands : ContentPage
{
    ...
    /// </summary>
    public PairsAndSoftHands(Account account, FirebaseAuthClient authClient)
    {
        InitializeComponent();
        _account = account;
        _authClient = authClient;
        MainBack.SetValue(Grid.ZIndexProperty, 2);
        PairsHeader.SetValue(Grid.ZIndexProperty, 1);
    ...
    private void DealStartCards()
    {
        ...
        BlackjackCard playerCard2 = playerCard1;
        if (playerCard1.Value == 10 && playerCard2.Value == 10)
        {
            TensCount++;
        }
    ...
    private void DealMoreCards()
    {
        ...
        if (RoundsPlayed % 2 == 1)
        {
            ...
            CardSuit randomSuit = (CardSuit)rand.Next(0, 4);
            playerCard2 = new BlackjackCard(CardFace.A, randomSuit);
            do
            {
        ...
        {
            if (TensCount/RoundsPlayed > 1.0/26)
            {
                do
                {
                    playerCard1 = (BlackjackCard)_deck.Deal();
                }
                while (playerCard1.Value == 10);
